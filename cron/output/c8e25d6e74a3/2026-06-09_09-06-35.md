...
执行 `prepare-digest.js` 时，`fetch` 请求全部失败。经过系统排查，确认是**网络层面的系统性阻断**：

| 检测项 | 结果 |
|---|---|
| `raw.githubusercontent.com` DNS 解析 | `198.18.0.82`（TEST-NET-2 保留地址段） |
| `github.com` DNS 解析 | `198.18.0.23` |
| `cdn.jsdelivr.net` DNS 解析 | 同样被污染 |
| `gh-proxy.com` DNS 解析 | 同样被污染 |
| `curl` 直连 | `SSL_ERROR_SYSCALL` |
| `web_extract` | `Blocked: URL targets a private or internal network address` |
| `web_search` | `Web tools are not configured` |
| 外部 DNS（8.8.8.8 / 1.1.1.1） | 超时不可达 |

**结论**：当前运行环境的所有外部域名 DNS 解析被劫持至 `198.18.0.0/15`（IETF 保留测试网段），所有出站 HTTPS 连接均不可达。这不是临时故障，而是环境级别的网络限制。

## 曾经成功的运行

`/tmp/fb-digest-2026-06-08.md` 显示上一次成功推送是 6 月 8 日。此后网络环境发生了变化。

## 建议修复方向

1. **检查 Hermes Agent 运行环境的网络配置**：确认是否切换了网络、VPN 断开、或防火墙规则变更
2. **配置 HTTP 代理**：如果环境需要通过代理访问外网，在 `~/.hermes/config.yaml` 中或通过环境变量 `HTTPS_PROXY` 配置代理
3. **检查 DNS 设置**：`198.18.0.0/15` 是某些网络管理软件（如 Cisco Umbrella/OpenDNS 的 blocking page）使用的地址，确认是否有安全策略变更
4. **临时绕过**：如果可以 SSH 到宿主机，尝试 `sudo killall -HUP mDNSResponder` 刷新 DNS 缓存

---

**今天无法生成摘要内容的根本原因不在 Hermes 或 follow-builders skill**，而在于宿主机网络层的 DNS 劫持。恢复网络连接后，下次 cron 触发将自动恢复正常推送。
