...
## 🚨 follow-builders 每日摘要 — 网络阻塞诊断报告

### 诊断结果

prepare-digest.js 返回 `{"status":"error","message":"fetch failed"}`。经系统排查，确认是**全网 DNS 污染 + TCP 层封锁**导致完全无法访问内容源：

| 检查项 | 结果 | 说明 |
|--------|------|------|
| `raw.githubusercontent.com` DNS | `198.18.0.82` | ❌ IETF TEST-NET-2 保留地址（DNS 污染） |
| `cdn.jsdelivr.net` DNS | `198.18.0.176` | ❌ 同上，CDN 镜像也被污染 |
| `github.com` DNS | `198.18.0.23` | ❌ 所有 GitHub 域名均被重定向到虚假 IP |
| CDN 镜像 TCP 连接 | `SSL_ERROR_SYSCALL` | ❌ TCP 层完全封锁，非仅 DNS 问题 |
| 代理配置 | 未设置 | ❌ 无 HTTP/HTTPS 代理可用 |
| 缓存内容 | `/tmp/fb-digest-2026-06-11.md` (昨日) | ⚠️ 仅有昨日缓存，无今日新数据 |

### 根因

当前网络环境对 GitHub 相关基础设施（`raw.githubusercontent.com`、`cdn.jsdelivr.net`、`github.com`）实施了全量 DNS 污染 + TCP 封锁。所有域名均被解析到 `198.18.0.0/15` 网段（IETF 测试保留地址），且 TCP 443 端口连接被 `SSL_ERROR_SYSCALL` 拒绝。这是典型的防火墙/OpenDNS/Cisco Umbrella 级别的封锁行为。

### 建议

1. **配置 VPN 或代理**：设置 `HTTPS_PROXY` 环境变量后重试
2. **检查网络防火墙**：确认是否有企业级 DNS 过滤器（如 Cisco Umbrella）在运行
3. **使用移动热点**：临时切换网络源测试是否仅为当前 WiFi 限制
