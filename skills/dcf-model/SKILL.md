...
**WITHOUT the column header row, the model is broken** because each scenario's assumptions cannot be mapped to projection years. This is a common error. Always include the column header row.

### Correct Case Selector Implementation

**Never embed scenario logic in individual projection formulas.** Instead:

1. **Create a case selector** in a dedicated cell (e.g., B6)
2. **Create scenario assumption blocks** for Bear/Base/Bull with assumptions across years
3. **Build a consolidation column** (typically the next column to the right) that uses INDEX formulas to pull from the selected scenario block based on the case selector. This consolidation column is what your projection formulas reference.
