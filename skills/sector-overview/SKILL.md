...
## Chinese Industry Deep-Dive Reports (A-share)

For Chinese-language industry reports following broker research format (券商研报), see `references/cn-industry-report-workflow.md`. Covers:

- Standard report structure (摘要 → 行业概述 → 政策 → 产业链 → 发展现状 → 痛点 → 展望 → 地方产业 → 重点公司)
- Citation format: "来源：机构名称，中泰证券研究所"
- Reference tracker Excel creation with multi-sheet source attribution
- Government website (.gov.cn) WAF workarounds
- Screenshot fallback techniques for source verification

Trigger keywords: 行业报告、行业深度、产业研究、造纸/铝/新能源等具体行业名 + 报告/研报/分析

Writing style guidance in `references/cn-industry-report-workflow.md` §5 covers paragraph density control, four-layer structure template, data simplification rules, and equity research tone conventions.
